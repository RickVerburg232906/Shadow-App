...
// This will create/update two users and set custom claims.
// Requires serviceAccountKey.json in project root (Firebase service account).
...
async function upsert(email, password, role) {
  try {
    ...
    // set custom claim with 1 hour expiry
...
(async () => {
  try {
    ...
    // Also update Firestore globals/passwords for compatibility with older UI
    try {
